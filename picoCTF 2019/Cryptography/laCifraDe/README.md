...
## Solution
When we connect to the server, we get the following text:
```
Encrypted message:
...
As the hint tells us to look at history, we googled "1508 cipher" and found "Vigenere Cipher". Next, we try to use an [online tool](https://www.guballa.de/vigenere-solver) to solve the text (there are a lot more tools online but not all of them works).

Turns out, the key is 'flag' and the decrypted text is as follow:
```
